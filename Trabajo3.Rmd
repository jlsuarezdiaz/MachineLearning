...
calculateEin <- function(ml ,ltrain){
  ...
  return(list(ml.Ein, ml_pred_train))
}
...
models <- c(ml.binomial1, ml.binomial2, ml.binomial3, ml.gaussian1, ml.gaussian2, ml.gaussian3, ml.poisson1, ml.quasi1, ml.quasibinomial1, ml.quasipoisson1)
...
testFamilies <- function(models, test, ltrain, ltest){
  ...
  for(i in seq_along(models)){
    results[i,1] <-calculateEin(models[i],ltrain)
    results[i,2] <- calculateEout(models[i],test,ltest)
  }
...
#Cálculo de probabilidades
ml3.prob_train = predict(ml3, type = "response") #no tenemos que introducirle el train porque recuerda
ml3.prob_test = predict(ml3, sahd.test, type="response")

# Etest
ml3.pred_test = rep(0, length(ml3.prob_test)) # predicciones por defecto 0
ml3.pred_test[ml3.prob_test >=0.5] = 1 # >= 0.5 clase 1

table(ml3.pred_test, lsahd.test) 

ml3.Etest = mean(ml3.pred_test != lsahd.test)
cat("Etest con el modelo Gaussiano: "); 
print(ml3$call)
print(ml3.Etest)
    
# Ein
ml3.pred_train = rep(0, length(ml2.prob_train)) #predicciones por defecto 0
ml3.pred_train[ml3.prob_train >= 0.5] = 1
table(ml3.pred_train, lsahd.train)
    
ml3.Ein = mean(ml3.pred_train != lsahd.train)
cat("Ein con el modelo de Poisson: "); 
print(ml3$call)
print(ml3.Ein)
...
#Cálculo de probabilidades
ml4.prob_train = predict(ml4, type = "response") #no tenemos que introducirle el train porque recuerda
ml4.prob_test = predict(ml4, sahd.test, type="response")

# Etest
ml4.pred_test = rep(0, length(ml1.prob_test)) # predicciones por defecto 0
ml4.pred_test[ml1.prob_test >=0.5] = 1 # >= 0.5 clase 1

table(ml4.pred_test, lsahd.test) 

ml4.Etest = mean(ml4.pred_test != lsahd.test)
