...
```{r, echo=F, warning=F}
  ...
  # ESTO NOS HACE FALTA YA ??? !!! -> Siii, porque luego para ir imprimiendo cosas llamamos a metodos sobre esto
...
- *lambda.min*, que nos devuelve el valor del menor $\lambda$ obtenido.
- *lambda.1se*, valor que se obtiene, tras calcular el modelo que proporciona menor error, para el mejor modelo que tiene su error "within one standard error of that minimum error" (no entiendo esta frase !!!) (1se significa one-stand-error) ??????????
...
En este punto nos planteamos cuántos atributos utilizar para nuestro modelo. Claramente, cuantos más atributos utilicemos menor será el error producido (solo Ein no ???!!!). Esto lo podemos corroborar dibujando la gráfica del error por mínimos cuadrados en función del número de atributos elegidos.
...
Como en el problema de regresión, procedemos a procesar los datos para reducir asimetrías, escalarlos y centrarlos. De nuevo, como trabajamos con un número pequeño de atributos, no aplicaremos PCA para la reducción de atributos.
...
```{r, echo=F,warning=F}
  ...
  grid=10^seq(10,-2,length=100)
  mr_lasso <- cv.glmnet(as.matrix(ozone.train), lozone.train, lambda = grid)
...
En este caso vemos que sí hay transformaciones polinómicas que mejoran el ajuste.
