...
Para el problema de clasificación hemos elegido la base de datos de *South African Heart Disease*, que almacena una muestra recapituladora de hombres en alto riesgo de cardiopatía en la región de Western Cape, en Sudáfrica. Hay dos casos de CHD (0 o 1). Algunos de los hombres que tienen CHD positivo, se han sometido a un tratamiento de reducción de la presión sanguínea, siendo los datos que aparecen en la muestra posteriores a estos tratamientos. Esta información data de 1983.
...
- **doy:** Día del año en el que se realizó la medición.
...
Un último detalle sobre los atributos es que, en un principio podría pensarse que `doy` (day of year) carece de importancia a la hora de estimar `ozone`. Sin embargo, tras analizar más profundamente el atributo, vemos que lo que representa es de gran utilidad, puesto que cada posible valor hace referencia a un único día de un año natural, y en este aspecto (y además se puede apreciar en las gráficas anteriores), el nivel de ozono sí que depende la época (estaciones, temperaturas, lluvias) del año en que se mide.
...
```{r, echo=F,warning=F}
  ...
  #Obtenemos una tabla con los que es mejor coger.
  stesting <- summary(testing)
  print(stesting$outmat)
```

En la tabla anterior vemos por filas cuáles son las mejores combinaciones de $n$ atributos. El valor $n$ es el que aparece al inicio de cada fila, y los atributos que entran en su respectiva combinación los marcados con el símbolo `*`.
...
En este caso, a diferencia del conjunto de clasificación, para los distintos modelos, número de atributos o transformaciones seleccionadas sí que se aprecian mejoras en los valores obtenidos del error. El conjunto de datos utilizado para la regresión se presta más a mejorar que el utilizado en clasificación.

Por otra parte, aunque el modelo escogido presenta ventajas en cuanto al número de atributos (de acuerdo al criterio `BIC`), el modelo con todos los atributos es el que produce mejores resultados, tras aplicar transformaciones no lineales (en este caso polinómicas) sobre los atributos. Además, vemos que los atributos `vh`, `ibh`, `dpg` y `doy` son los que cambian con el algoritmo a comportamiento cuadrático. Esto nos confirma lo que se podía intuir viendo la gráfica de la sección de Preprocesamiento de Datos, en la que cuando se ponía `ozone` en función de cada uno de estos atributos, la distribución de puntos no parecía ser lineal. Finalmente, la transformación del atributo `doy` nos confirma lo que ya comentamos previamente sobre su importancia, puesto que no solo no es eliminado en la reducción de atributos con `regSubsets`, sino que llega a influir de forma cuadrática en la regresión. Para concluir en el tema de las transformaiciones, concluimos que, en comparación con el problema de clasificación, en este caso optar por un aumento en la complejidad de los atributos sí que nos ha proporcionado mejoras significativas, por lo que en este caso sí lo elegimos antes que la transformación lineal.

Finalmente, en este caso la regularización, aunque presenta mejoras, son insignificantes en las múltiples ejecuciones realizadas. Por tanto, la necesidad de regularización tampoco se aprecia en el modelo de regresión.
