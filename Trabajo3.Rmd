...
```{r, echo=F,warning=F}
  ...
  st(ml,sahd.test,lsahd.test,ml_lasso$lambda.min)[[1]]
  
  st(ml,sahd.test,lsahd.test,ml_lasso$lambda.1se)[[1]]
  
  st(ml.sahd,sahd.test,lsahd.test)[[1]]
  
  # Los tiros pueden ir por aquí para el Ereg
  predict(ml_lasso,as.matrix(sahd.test),type = "response", s = ml_lasso$lambda.min)
