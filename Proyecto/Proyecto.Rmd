...
#har.train <- har.train + 1.1
#Transformamos los datos para poder aplicar BoxCox, los transladamos al [0,2]

ObjetoTrans.train <- preProcess(har.train,method = c("pca"), 0.8)
ObjetoTrans.test <- preProcess(har.test, method = c("pca"), 0.8)
...
har.trans.train <- predict(ObjetoTrans.train, har.train)
har.trans.test <- predict(ObjetoTrans.test, har.test)
...
```{r, echo=F, warning=F}
  ...
  #Ahora los escalamos y centramos
  ObjetoTrans.train <- preProcess(har.train,method = c("center", "scale"))
  ObjetoTrans.test <- preProcess(har.test,method = c("center", "scale"))
