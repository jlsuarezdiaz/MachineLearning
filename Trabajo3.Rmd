...
calculateEtestReg <-function(datos,label,pesos){
  suma = 0;
...
testPolyTransform <- function(data,label){
  ...
  ml <- glm(chd ~., family = gaussian(log), data = data, subset =train)
...
polynomialTransformGreedy <- function(data,label,max_coeff){
  transform <- data
  coeffs <- c()
  ...
  for(i in 1:ncol(transform)){
    ...
    for(j in 1:max_coeff){
      ...
      v <- testPolyTransformRep(10,transform,label)
...
```{r, echo=F,warning=F}
polynomialTransformGreedy(sahd.data,sahd.label,4)
