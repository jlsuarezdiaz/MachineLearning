...
st <- function(ml, test, ltest, s=0){
  ...
  if(s==0){
    ...
    ml.prob_test = predict(ml, test, type = "response", s = s)
    print(ml.prob_test$)
  }
...
>>>>>>> f31746a3418aec793b242278e6482f1096384f3f
...
#quasipoisson
ml.quasipoisson1 <- glm(chd ~ PC1, family = quasipoisson(link = "log"), data = sahd.data, subset=train)  
...
```{r, echo=F,warning=F}
  ...
  coeffs <- rep(288,length(coefficientes.1se))
  
  st(ml.sahd,sahd.test,lsahd.test,ml_lasso$lambda.min)
  
  st(ml.sahd,sahd.test,lsahd.test,ml_lasso$lambda.1se)
  
  st(ml.sahd,sahd.test,lsahd.test)
